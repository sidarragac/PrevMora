...
from ..schemas.Client import (
    ClientResponse, ClientCreate, ClientUpdate, ClientList, 
    ClientCompleteResponse, CreditDetailResponse, InstallmentDetailResponse,
    PortfolioDetailResponse, AlertDetailResponse, ReconciliationDetailResponse
)
...
class ClientController(BaseController):
    ...
    async def get_client_complete_data(self, session: AsyncSession, client_id: int) -> ClientCompleteResponse:
        """
        ...
        query = select(Client).where(Client.id == client_id).options(
            selectinload(Client.credit).selectinload(Credit.installment).selectinload(Installment.portfolio),
        ...
        for credit in client.credit:
            ...
            for installment in credit.installment:
                ...
                for portfolio in installment.portfolio:
                    ...
                    portfolio_data.append(PortfolioDetailResponse.model_validate(portfolio))
                
                installment_response = InstallmentDetailResponse.model_validate(installment)
        ...
        alerts_data = [AlertDetailResponse.model_validate(alert) for alert in client.alert]
        ...
        if payment_references:
            ...
            reconciliations_data = [ReconciliationDetailResponse.model_validate(rec) for rec in reconciliations]
