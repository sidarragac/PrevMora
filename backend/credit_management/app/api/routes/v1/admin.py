    ...
    try:
        async with sessionmanager.session() as session:
            ...
            return {
                ...
                "missing_tables": [
                    t for t in defined_tables if t not in existing_tables
                ],
                "extra_tables": [t for t in existing_tables if t not in defined_tables],
            }
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Error verificando tablas: {str(e)}"
        )
...
async def debug_foreign_keys():
    try:
        ...
        for table_name, table in Base.metadata.tables.items():
            ...
            if table_fks:
                fk_info.append({"table": table_name, "foreign_keys": table_fks})
