...
from ..schemas.Payment import PaymentInitializationRequest, PaymentInitializationResponse


class PaymentController:
    """Controller for Payment operations."""
    
    PAYMENT_GATEWAY_URL = "https://payment-gateway3-beige.vercel.app/api/initialize-payment"
    ...
    async def initialize_payment(
        ...
        try:
            ...
            raise HTTPException(
                status_code=404,
                detail=f"Failed to get credit details: {e.detail}"
            )
        ...
        if len(credits_data) == 0:
            raise HTTPException(
                status_code=404,
                detail="No credits found for this client"
            )
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                await self._auto_process_pending_installments(
                    session, 
                    credit_to_send.id,
                    credit_to_send.payment_reference
                )
    ...
    def _convert_to_payment_gateway_format(self, credit_data) -> dict:
        ...
        for installment in credit_data.installments:
            installment_dict = {
                ...
                "installments_value": str(installment.installments_value),  # Convert to string
        ...
        payload = {
            ...
            "total_paid": credit_data.total_paid if credit_data.total_paid is not None else 0,
            "total_pending": credit_data.total_pending if credit_data.total_pending is not None else 0
        }
    ...
    async def _auto_process_pending_installments(
        ...
        try:
            ...
            all_paid = all(inst.installment_state == "Pagada" for inst in all_installments)
